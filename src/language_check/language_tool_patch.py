...
def _query_server_post(
    ...
    """
    if DEBUG_MODE:
        print('_query_server_post url:', url, 'params keys:', list(params.keys()) if params else None)
        ...
        try:
            ...
            with requests.post(
                ...
                try:
                    ...
                    if DEBUG_MODE:
                        print(
                            f'URL {url} returned invalid JSON response: {e}'
                        )
                        print(response)
...
def apply_post_request_patch() -> None:
    ...
    if not hasattr(lt_server.LanguageTool, '_query_server_original'):
        lt_server.LanguageTool._query_server_original = lt_server.LanguageTool._query_server
...
def revert_post_request_patch() -> None:
    ...
    if hasattr(lt_server.LanguageTool, '_query_server_original'):
        lt_server.LanguageTool._query_server = lt_server.LanguageTool._query_server_original
